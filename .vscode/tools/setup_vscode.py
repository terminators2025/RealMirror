...
parser.add_argument("--isaac_lab_path", type=str, help="The absolute path to the Isaac Lab project.")
...
if not os.path.exists(args.isaac_lab_path):
    raise FileNotFoundError(
        f"Could not find the Isaac Lab directory: {args.isaac_lab_path}. Please provide the correct path to the"
        " Isaac Lab project."
    )
...
def overwrite_python_analysis_extra_paths(isaaclab_settings: str) -> str:
    ...
    if os.path.exists(isaacsim_vscode_filename):
        ...
        if settings is not None:
            ...
            # if pattern not found, use empty list
            path_names = []
            print(
                "[WARN] Could not find 'python.analysis.extraPaths' in Isaac Sim VSCode settings."
                "\n\tUsing empty list for extra paths."
            )
    ...
    isaaclab_extensions = os.listdir(os.path.join(ISAAC_LAB_DIR, "source"))
    # make Isaac Lab source paths relative to the project directory (consistent with ISAACSIM handling)
    rel_lab_path = os.path.relpath(ISAAC_LAB_DIR, PROJECT_DIR)
    path_names.extend(['"${workspaceFolder}/' + rel_lab_path + "/source/" + ext + '"' for ext in isaaclab_extensions])
